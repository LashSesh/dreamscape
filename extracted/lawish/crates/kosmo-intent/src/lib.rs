...
use kosmo_core::{
    ...
    ParseBackScanScope, Wish, WishAssessment, WishConvergenceTrace, WishCube, WishFacet,
    WishFacetKind, WishPredicate, Q16,
};
...
fn collect_js(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            collect_js(&path, out);
        } else if path.to_str().and_then(SourceLanguage::from_path)
...
mod tests {
    ...
    fn observe_layered_accumulates_cubes_and_keeps_flat_trace() {
        ...
        session.observe_layered(&ObservedTopology::from_facets([WishFacet::crate_(
            "kosmo-api",
        )]));
    ...
    fn mesh_reading_id_tracks_d_density() {
        ...
        assert_ne!(
            m1.id, m3.id,
            "a different topology gear → a different reading"
        );
    }
