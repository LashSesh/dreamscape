...
mod tests {
    ...
    fn rediscovery_is_rejected_but_a_true_mixture_is_admitted() {
        ...
        assert!(
            !inv.is_novel(&combine(&f, &f)),
            "fire + fire = fire is no discovery"
        );
        ...
        assert!(
            inv.admit(combine(&f, &t)),
            "the mixture is admitted as a discovery"
        );
    ...
    fn saturation_grows_a_catalog_from_primitives_and_terminates() {
        ...
        assert_eq!(
            inv.saturate(16, 256),
            0,
            "the catalog has closed (a fixpoint)"
        );
    }
    ...
    fn validity_gate_off_by_default_admits_scaffolding() {
        ...
        assert!(
            inv.admit(elem("import", 0, 0, 1, 0)),
            "novelty-only admits scaffolding"
        );
    ...
    fn armed_validity_gate_rejects_pure_scaffolding() {
        ...
        assert!(
            !inv.admit(elem("import", 0, 0, 1, 0)),
            "defines nothing → rejected"
        );
        assert!(
            !inv.admit(elem("test", 0, 0, 0, 1)),
            "defines nothing → rejected"
        );
        assert!(
            inv.admit(elem("func", 1, 0, 0, 0)),
            "defines a function → admitted"
        );
        assert!(
            inv.admit(elem("mixed", 1, 0, 2, 3)),
            "substance + scaffolding → admitted"
        );
        assert_eq!(inv.len(), 2);
