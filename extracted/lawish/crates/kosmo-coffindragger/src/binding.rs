...
mod tests {
    ...
    fn only_a_fully_bound_artifact_is_pullable() {
        ...
        assert!(
            !bound(Digest::ZERO, tr, true, Status::Pass).is_pullable(),
            "no evidence"
        );
        assert!(
            !bound(ev, Digest::ZERO, true, Status::Pass).is_pullable(),
            "no trace"
        );
        assert!(
            !bound(ev, tr, false, Status::Pass).is_pullable(),
            "not replay-aware"
        );
        assert!(
            !bound(ev, tr, true, Status::Reject).is_pullable(),
            "rejected status"
        );
    }
    ...
    fn binding_a_real_systemcube_yields_an_evidence_bound_stage() {
        ...
        let manifest = SystemCubeManifest::new(Digest::of_bytes(b"host"), &run, &policy, &units);
        ...
        assert_eq!(
            stage.units.len(),
            2,
            "two accepted units become the support"
        );
