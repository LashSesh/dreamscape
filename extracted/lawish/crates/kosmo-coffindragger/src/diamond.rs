...
pub fn press_diamond(
    ...
    if !replay {
        return Err(GateError::ReplayIncomplete(
            "diamond requires replay".into(),
        ));
    }
    if !irreducible {
...
mod tests {
    ...
    fn bundle() -> FoldBundle {
        FoldBundle::fold(
            &[] as &[Stage],
            vec![Digest::of_bytes(b"u")],
            vec![Digest::of_bytes(b"r")],
        )
    }
    fn cert() -> Digest {
    ...
    fn a_fully_certified_irreducible_core_presses_a_diamond() {
        ...
        assert_eq!(
            cube.materialization_profile,
            MaterializationProfile::ReportOnly
        );
