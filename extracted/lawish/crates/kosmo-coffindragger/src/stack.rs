...
mod tests {
    ...
    fn covert_removal_fails_accretion_but_accounted_removal_passes() {
        ...
        assert_eq!(
            verify_accretion(&prev, &[d(b"a"), d(b"b")], &[], &[]),
            Status::Reject
        );
        // c dropped but recorded as residue → pass (cleaning is visible).
        assert_eq!(
            verify_accretion(&prev, &[d(b"a"), d(b"b")], &[], &[d(b"c")]),
            Status::Pass
        );
    ...
    fn contraction_must_not_grow_and_must_account_for_loss() {
        ...
        assert_eq!(
            verify_contraction(&before, &[d(b"a"), d(b"b")], &[d(b"c")]),
            Status::Pass
        );
        // grew → not a contraction.
        assert_eq!(
            verify_contraction(&before, &[d(b"a"), d(b"b"), d(b"c"), d(b"x")], &[]),
            Status::Reject
        );
        // shrank but dropped unit unaccounted → reject (negative test 5).
        assert_eq!(
            verify_contraction(&before, &[d(b"a")], &[d(b"b")]),
            Status::Reject
        );
    }
    ...
    fn a_qsr_monotone_closed_stack_emits_a_diamond_candidate() {
        ...
        match close_stack(&stages, vec![d(b"a"), d(b"b")], vec![], true) {
            ClosureOutcome::DiamondCandidate(b) => {
                assert!(
                    b.stage_ids.contains(&stages[1].canon()),
                    "Canon(B_N) = Canon(S_N)"
                );
            }
    ...
    fn a_pure_copy_is_not_an_assimilation_and_drift_needs_an_explanation() {
        ...
        assert_eq!(
            verify_assimilation(&s, &s.clone(), &Delta::default()),
            Status::Reject
        );
        let delta = Delta {
        ...
        assert_eq!(
            verify_assimilation(&s, &s, &delta),
            Status::Pass,
            "a real fragment assimilates"
        );
        ...
        assert_eq!(
            verify_roundtrip(&s, &other, false),
            Status::Reject,
            "unexplained drift"
        );
        assert_eq!(
            verify_roundtrip(&s, &other, true),
            Status::Warn,
            "explained drift is downgraded"
        );
    }
    ...
    fn a_stack_folded_without_replay_is_not_closed() {
        ...
        assert_eq!(
            stack_closed(&bundle, &s0, Status::Pass, false),
            Status::Reject
        );
    }
}
