...
use kosmo_systemcube::{
    BlueprintUnit, BlueprintUnitKind, ContradictionEnergyReport, SystemCubeManifest,
};
...
fn live_fold(path: &str) -> Option<(Vec<Stage>, Vec<Digest>, Vec<Digest>, f64, f64)> {
    ...
    let manifest =
        SystemCubeManifest::new(Digest::of_bytes(path.as_bytes()), &run, &policy, &units);
    let k =
        ContradictionEnergyReport::from_units(manifest.manifest_id, &policy, &units).total_energy;
...
fn run(verb: &str, json: bool, workspace: Option<&str>) -> ExitCode {
    ...
    match verb {
        ...
        "stack" => {
            ...
            println!(
                "kosmo-cdk stack — {source} · {} stage(s), stack-QSR: {qsr:?}",
                stages.len()
            );
        ...
        "close" => {
            ...
            match close_stack(&stages, support, residue, true) {
                ...
                ClosureOutcome::DeferredStackReport { bundle, .. } => {
                    println!(
                        "kosmo-cdk close — DEFERRED · bundle {}",
                        hex(&bundle.bundle_id)
                    );
                    ExitCode::from(1)
                }
            }
                ...
                match press_diamond(
                    ...
                    Ok(cube) => {
                        if json {
                            ...
                            println!(
                                "  support_units     {}  (the irreducible core)",
                                cube.support_units.len()
                            );
                            println!(
                                "  residue_report    {}  (the exkalibrated duplicate)",
                                hex(&cube.residue_report_id)
                            );
        ...
        "explain" => {
            let qsr = qsr_stack(&stages);
            println!(
                "kosmo-cdk explain — the fold over a real SystemCube, gate by gate (report-only):"
            );
            ...
            println!(
                "  · residue (visible): {} unit(s); ids canonical (I1); trace roots present (I4)",
                residue.len()
            );
...
fn rest_artifact(verb: &str, id: Option<&str>) -> serde_json::Value {
    ...
    match verb {
        ...
        "stack" => {
            let qsr = qsr_stack(&stages);
            wrap(
                &format!("{qsr:?}").to_lowercase(),
                json!({ "stages": stages.len(), "qsr_status": format!("{qsr:?}") }),
            )
        }
        "close" => match close_stack(&stages, support, residue, true) {
            ...
            ClosureOutcome::DeferredStackReport { bundle, .. } => wrap(
                "defer",
                json!({ "closed": false, "fold_bundle": bundle.bundle_id.to_hex() }),
            ),
        },
        "diamond" => match close_stack(&stages, support, residue, true) {
            ClosureOutcome::DiamondCandidate(bundle) => {
...
mod tests {
    ...
    fn rest_diamond_carries_trace_gate_and_the_cube() {
        ...
        assert!(
            v["trace_root"].is_string(),
            "every response carries a trace_root"
        );
    ...
    fn observe_workspace_reads_real_crates_live() {
        ...
        assert_eq!(
            units.len(),
            2,
            "two real crates observed from the filesystem"
        );
        assert!(
            live_fold(dir.to_str().unwrap()).is_some(),
            "a live fold over the workspace"
        );
