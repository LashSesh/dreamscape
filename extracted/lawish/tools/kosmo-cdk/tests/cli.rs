...
fn diamond_emits_a_qsr_certified_candidate() {
    ...
    assert!(
        out.status.success(),
        "the sample fold closes to a diamond: {stdout}"
    );
    assert!(stdout.contains("DiamondCubeCandidate"), "{stdout}");
    assert!(
        stdout.contains("qsr_certificate"),
        "carries a QSR certificate: {stdout}"
    );
    assert!(
        stdout.contains("ReportOnly"),
        "L8 safety: report-only by default: {stdout}"
    );
}
...
fn diamond_json_is_a_complete_candidate() {
    let out = cdk()
        .args(["diamond", "--json"])
        .output()
        .expect("spawn kosmo-cdk");
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).expect("valid JSON");
    assert!(v["diamond_id"].is_string());
    assert!(
        v["qsr_certificate_id"].is_string(),
        "QSR certificate present"
    );
