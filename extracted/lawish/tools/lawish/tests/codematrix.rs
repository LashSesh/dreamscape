...
fn codematrix_renders_a_holistic_polyglot_cube() {
    ...
    assert!(
        stdout.contains("3 source"),
        "all three languages are fingerprinted: {stdout}"
    );
    assert!(
    ...
    assert!(
        stdout.contains("homogeneity"),
        "with a homogeneity scalar: {stdout}"
    );
    assert!(
...
fn codematrix_json_carries_the_holistic_reading() {
    ...
    assert!(
        holistic["axes"].is_array(),
        "holistic axes present: {stdout}"
    );
    assert_eq!(
        holistic["axes"].as_array().map(|a| a.len()),
        Some(5),
        "five axes: {stdout}"
    );
    assert!(
        holistic["homogeneity"].is_number(),
        "homogeneity present: {stdout}"
    );
    assert_eq!(
        v["cross_language"]["languages"].as_u64(),
        Some(3),
        "three languages: {stdout}"
    );
    assert!(
