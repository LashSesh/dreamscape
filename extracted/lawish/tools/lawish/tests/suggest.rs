...
fn unmet_wish_suggests_a_near_match() {
    ...
    assert!(
        ...
        "a near match is offered: {stdout}"
    );
    assert!(
        stdout.contains("engine"),
        "it names the real module: {stdout}"
    );
}
...
fn signature_arity_gap_is_bridged() {
    ...
    assert!(
        stdout.contains("did you mean"),
        "the arity form is suggested: {stdout}"
    );
    assert!(
        stdout.contains("add/2"),
        "names the arity-keyed signature: {stdout}"
    );
}
...
fn a_genuine_gap_gets_no_false_suggestion() {
    ...
    assert!(
        !stdout.contains("did you mean"),
        "no false suggestion for a real gap: {stdout}"
    );
}
