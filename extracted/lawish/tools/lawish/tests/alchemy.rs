...
fn alchemy_seeds_from_workspace_and_reaches_a_fixpoint() {
    ...
    assert!(
        ...
        "seeded from the workspace: {stdout}"
    );
    assert!(
        stdout.contains("discoveries"),
        "the discovery loop is reported: {stdout}"
    );
    assert!(
        stdout.contains("fixpoint reached"),
        "the catalog closes: {stdout}"
    );
}
...
fn alchemy_json_carries_the_catalog() {
    ...
    assert!(
        catalog >= distinct,
        "the catalog grows from its seeds: {catalog} >= {distinct}"
    );
...
fn alchemy_finer_threshold_resolves_a_larger_catalog() {
    let root = workspace();
    let coarse = alchemy_json(&root, Some("0.80"))["catalog"]
        .as_u64()
        .unwrap();
    let fine = alchemy_json(&root, Some("0.97"))["catalog"]
        .as_u64()
        .unwrap();
    assert!(
        fine >= coarse,
        "a finer novelty threshold resolves more: fine={fine} coarse={coarse}"
    );
}
...
fn alchemy_falls_back_to_primitives_without_source() {
    ...
    assert_eq!(
        v["seeded_from"], "primitives",
        "no source → the four primitives"
    );
    assert_eq!(
        v["distinct_elements"].as_u64(),
        Some(4),
        "four primitive elements"
    );
    assert!(
        v["discovered"].as_u64().unwrap() > 0,
        "combination still discovers"
    );
}
...
fn alchemy_is_one_door_per_run() {
    ...
    assert!(
        !out.status.success(),
        "two organ doors in one run is rejected"
    );
}
