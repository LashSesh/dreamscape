...
fn since_names_the_newly_realized_facet() {
    ...
    assert!(
        out.contains("Module beta"),
        "the delta names the facet: {out}"
    );
    // A pure gain to a now-realized wish exits 0 — no regression.
    assert_eq!(
        delta.status.code(),
        Some(0),
        "realized, no regression: {out}"
    );
}
...
fn since_regression_exits_2() {
    ...
    assert_eq!(
        snap.status.code(),
        Some(0),
        "both modules present → realized: {snap_out}"
    );
    ...
    assert!(
        stdout.contains("Module beta"),
        "names the broken facet: {stdout}"
    );
}
...
fn since_json_emits_a_self_describing_machine_delta() {
    ...
    assert!(
        v.get("gained").is_some(),
        "machine delta carries gained: {stdout}"
    );
    assert!(v.get("regressed").is_some(), "and regressed: {stdout}");
    assert!(
        v.get("wish_id").is_some(),
        "self-describing by wish id: {stdout}"
    );
}
