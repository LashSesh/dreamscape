...
fn the_instrument_measures_a_python_workspace() {
    ...
    assert!(
        ...
        "the Python facets are all present: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(0),
        "a realized polyglot wish exits 0: {stdout}"
    );
}
...
fn an_absent_python_facet_is_unmet() {
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "an absent function is unmet: {stdout}"
    );
...
fn the_instrument_measures_a_javascript_workspace() {
    ...
    assert!(
        stdout.contains("REALIZED"),
        "the JavaScript facets are present: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(0),
        "a realized JS wish exits 0: {stdout}"
    );
}
