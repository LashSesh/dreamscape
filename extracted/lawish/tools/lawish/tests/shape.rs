...
fn doc_shape_is_unmet_for_an_undocumented_function() {
    ...
    assert!(
        stdout.contains("Doc beta"),
        "the doc gap is named: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(1),
        "an unmet doc wish exits 1: {stdout}"
    );
}
...
fn doc_shape_gap_is_closed_by_apply() {
    ...
    assert!(
        ...
        "--apply closes the doc gap: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(0),
        "realized after --apply: {stdout}"
    );
    let lib = fs::read_to_string(root.join("src/lib.rs")).unwrap();
    assert!(
        lib.contains("///"),
        "a doc comment was written for beta: {lib}"
    );
}
