...
// The `--ledger` recall path is an adapter dependency (pse-adapter-kosmo) gated
// behind the `ledger` feature in standalone lawish. This contract test exercises
// that adapter, so it only runs when the feature is built.
#[cfg(feature = "ledger")]
#[test]
