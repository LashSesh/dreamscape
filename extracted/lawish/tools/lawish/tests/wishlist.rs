...
fn kosmo_run() -> Command {
    Command::new(env!("CARGO_BIN_EXE_lawish"))
}
...
fn wishlist_reports_aggregate_and_gates_unrealized() {
    ...
    assert!(
        stdout.contains("realized 2/3"),
        "alpha + f met, beta missing: {stdout}"
    );
    assert!(stdout.contains("a module beta"), "{stdout}");
    assert_eq!(
        out.status.code(),
        Some(1),
        "not all realized → exit 1: {stdout}"
    );
}
...
fn wishlist_blueprint_reads_the_file_as_one_architecture() {
    ...
    assert!(
        ...
        "renders the city plan: {stdout}"
    );
    assert!(stdout.contains("the architecture"), "{stdout}");
    assert!(
        stdout.contains("2/3 realized"),
        "alpha + f met, beta missing: {stdout}"
    );
    assert!(
        stdout.contains("incomplete"),
        "the plan does not yet stand: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn wishlist_blueprint_json_emits_architecture_nodes() {
    ...
    assert!(
        v.is_array(),
        "the blueprint machine view is an array of nodes: {stdout}"
    );
    assert!(
...
// The `--plan` collapse-plan path is an adapter dependency (pse-traverse) gated
// behind the `plan` feature in standalone lawish, so this test only runs when
...
fn wishlist_plan_renders_collapse_plan_and_excises_a_heap() {
    ...
    assert!(
        stdout.contains("collapse plan"),
        "renders the plan: {stdout}"
    );
    assert!(
        stdout.contains("pse-traverse"),
        "names the engine: {stdout}"
    );
    assert!(
...
fn wishlist_blueprint_flags_a_dangling_wire() {
    ...
    assert!(
        ...
        "names the dangling endpoint: {stdout}"
    );
}
...
fn wishlist_blueprint_flags_a_never_run_city() {
    ...
    assert!(
        stdout.contains("the plan stands"),
        "structural city stands: {stdout}"
    );
    assert!(
...
fn wishlist_insist_rejects_a_broken_architecture() {
    ...
    assert!(
        stdout.contains("not accepted"),
        "a cycle is refused under --insist: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(3),
        "rejection is the distinct exit 3: {stdout}"
    );
}
...
fn wishlist_since_gates_a_project_regression() {
    ...
    assert_eq!(
        snap.status.code(),
        Some(0),
        "baseline all realized: {snap_out}"
    );
    ...
    assert!(
        stdout.contains("a module alpha"),
        "names the broken wish: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(2),
        "a project regression exits 2: {stdout}"
    );
}
...
fn wishlist_since_reports_progress_without_regression() {
    ...
    assert_eq!(
        out.status.code(),
        Some(0),
        "progress, no regression → exit 0: {stdout}"
    );
}
...
fn wishlist_scaffold_previews_the_plan_without_writing() {
    ...
    assert!(
        ...
        "the closure plan is previewed: {stdout}"
    );
    ...
    assert!(
        !root.join("src/delta.rs").exists(),
        "no files created by a dry run"
    );
}
...
fn wishlist_apply_closes_the_project() {
    ...
    assert!(
        stdout.contains("realized 2/2"),
        "the project was closed: {stdout}"
    );
    assert_eq!(
        out.status.code(),
        Some(0),
        "all realized after --apply → exit 0: {stdout}"
    );
...
fn wishlist_apply_shows_a_build_account() {
    ...
    assert!(
        stdout.contains("what --apply built"),
        "the build account renders: {stdout}"
    );
    assert!(
        stdout.contains("newly realized 1"),
        "engine was closed this run: {stdout}"
    );
    assert!(
        stdout.contains("a module engine"),
        "the account names what it built: {stdout}"
    );
}
...
fn wishlist_coverage_flags_a_structure_only_dod() {
    ...
    assert!(
        ...
        "the DoD coverage is shown: {stdout}"
    );
