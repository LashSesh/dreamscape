...
use kosmo_core::{
    ...
    PolicyProfile, PrecedenceOrder, RenderAnomaly, StageStanding, StagedClosureReport,
...
use kosmo_intent::{
    ...
    observe_workspace_validated, parse_atelier_command, AtelierCommand, ChatIntent,
    CubeMeshReading, DraftSlot, IndexSelection, IntentExtractor, KeywordIntentExtractor,
    NormCatalog, SuggestionSource, WishDraft, WishSession,
};
...
// `--ledger` adapter (classified: adapter dependency). Extracted lawish does not
// vendor `pse-adapter-kosmo`; the import and its single use site are gated so the
// default build is PSE-free. See EXTRACTION_REPORT.md.
#[cfg(feature = "ledger")]
...
fn open_recall(args: &Args) -> Result<Option<Arc<dyn MemoryRecall>>, String> {
    match &args.ledger {
        Some(p) => {
            ...
            {
                let _ = p;
                Err("--ledger requires building lawish with --features ledger \
                     (which vendors pse-adapter-kosmo from the source repo); \
                     the default standalone build is PSE-free"
                    .to_string())
            }
        }
        None => Ok(None),
...
impl WishlistDelta {
    fn compute(
        ...
        WishlistDelta {
            realized_now: current
                .iter()
                .filter(|(_, a)| is_realized(&a.status))
                .count(),
...
fn wishlist_delta_report(delta: &WishlistDelta, title: &str, color: bool) -> String {
    ...
    if delta.newly_realized.is_empty()
        && delta.regressed.is_empty()
        && delta.new_suspects.is_empty()
    {
        out.push_str(&format!(
...
fn vocab_report(color: bool) -> String {
    ...
    entry(
        &mut out,
        "a function <name>",
        "a function add  (also: type/struct/trait)",
    );
    ...
    entry(
        &mut out,
        "a run <fn>,<args>=>out~<expected>",
        "a run add,2,3=>out~5",
    );
    entry(
        &mut out,
        "a service <METHOD>:/<path>=><status>",
        "a service GET:/health=>200",
    );
    out.push_str(&format!(
...
fn run_wishlist_mode(args: &Args, path: &str) -> Result<ExitCode, String> {
    let text = std::fs::read_to_string(path)
        .map_err(|e| format!("could not read wishlist {path}: {e}"))?;
    ...
    if args.json {
        ...
        print!(
            "{}",
            wishlist_delta_report(d, "delta since baseline", args.color)
        );
    } else {
...
fn run_wish_mode(args: &Args) -> Result<ExitCode, String> {
    ...
    let delta = args.since.as_deref().and_then(|since| {
        load_prior_session(since, &wish).and_then(|prior| {
            prior
                .latest()
                .map(|base| WishDelta::compute(&wish, base, &assessment))
        })
    });
...
fn is_edge_kind(k: &WishFacetKind) -> bool {
    matches!(k, WishFacetKind::Dependency | WishFacetKind::Composition)
}
...
fn descend_to_wish(
    ...
    descend_inner(
        path, wish, evidence, validated, max_iters, fallback, prior, false, false,
    )
}
...
fn descend_staged(
    ...
    descend_inner(
        path, wish, evidence, validated, max_iters, fallback, prior, true, false,
    )
}
...
fn descend_guided(
    ...
    descend_inner(
        path, wish, evidence, validated, max_iters, fallback, prior, staged, true,
    )
}
...
fn insist_architecture_defect(nodes: &[BlueprintNode]) -> Option<String> {
    ...
    for e in nodes.iter().filter(|n| is_edge_kind(&n.kind)) {
        for ep in edge_component_endpoints(&e.kind, &e.key) {
            if !components.contains(ep.as_str()) {
                return Some(format!(
                    "dangling wire {} \u{2192} undeclared {}",
                    e.key, ep
                ));
            }
        }
...
fn layered_descent_report(session: &WishSession, mesh_density: Option<Q16>, color: bool) -> String {
    let c = |code: &'static str| if color { code } else { "" };
    let mut out = String::new();
    out.push_str(&format!(
...
mod tests {
    ...
    fn layered_report_shows_konus_focus() {
        ...
        let out = layered_descent_report(&session, None, false);
        assert!(
            out.contains("focus \u{2192}"),
            "render shows the lens focus: {out}"
        );
        assert!(
            out.contains("kosmo-api"),
            "focal foundation is the crate: {out}"
        );
    }
    ...
    fn honesty_verdict_calibrates_by_confidence() {
        ...
        assert!(
            suspect.contains("stub") && suspect.contains("live"),
            "{suspect}"
        );
        assert!(
        ...
        assert!(
            !thin.contains("over-fit"),
            "no alarm for a shallow wish: {thin}"
        );
        ...
        assert!(
            layered_descent_report(&s, Some(Q16::ratio(5, 100).unwrap()), false)
    ...
    fn blueprint_materializes_the_architecture_foundations_first() {
        ...
        assert!(
            nodes[0].stands_on.is_empty(),
            "the foundation stands on nothing"
        );
    ...
    fn blueprint_assessment_grades_coverage_and_honesty() {
        ...
        assert!(
            city_lines.contains("edge"),
            "an edge is connective tissue: {city_lines}"
        );
        assert!(
            !city_lines.contains("heap"),
            "no heap once wired: {city_lines}"
        );
        ...
        assert!(
            suspect.contains("suspect"),
            "deep+sparse standing city is suspect: {suspect}"
        );
        ...
        assert!(
            genuine.contains("genuine"),
            "deep+dense standing city is genuine: {genuine}"
        );
    }
    ...
    fn blueprint_wiring_integrity_flags_dangling_edges() {
        ...
        assert!(
            d.contains("dangling wire"),
            "edge to undeclared component: {d}"
        );
    ...
    fn blueprint_execution_coverage_flags_a_never_run_city() {
        ...
        assert!(
            s.contains("never run"),
            "a standing structure-only city is never run: {s}"
        );
        ...
        assert!(
            !e.contains("never run"),
            "an executed city is not 'never run': {e}"
        );
    }
    ...
    fn insist_rejects_a_broken_architecture_only() {
        ...
        assert!(
            insist_architecture_defect(&nodes_of(&cyclic)).is_some(),
            "a cycle is a defect"
        );
        ...
        assert!(
            insist_architecture_defect(&nodes_of(&dangling)).is_some(),
            "a dangling wire is a defect"
        );
        ...
        assert!(
            insist_architecture_defect(&nodes_of(&sound)).is_none(),
            "a sound architecture passes"
        );
    ...
    fn wish_delta_tracks_progress_and_regression() {
        ...
        assert!(
            r.contains("regressed 1") && r.contains("broke a met facet"),
            "{r}"
        );
    ...
    fn wishlist_report_aggregates_per_wish_standing() {
        ...
        assert!(
            out.contains("a crate solo") && out.contains("a crate ghost"),
            "{out}"
        );
        assert!(out.contains("spec.txt"), "names the source: {out}");
        assert!(
            !out.contains("suspect"),
            "no honesty flag without a grade: {out}"
        );
        assert!(
        ...
        assert!(
            flagged.contains("1 over-fit suspect"),
            "aggregate flag: {flagged}"
        );
        assert!(
            flagged.contains("\u{26a0} suspect"),
            "per-wish tag: {flagged}"
        );
    }
    ...
    fn closure_hint_triages_by_evidence() {
        ...
        assert_eq!(
            closure_hint(&[WishFacet::crate_("x")]),
            "scaffoldable offline"
        );
    ...
    fn nearest_existing_spots_typos() {
        ...
        assert!(
            out.contains("did you mean") && out.contains("engine"),
            "{out}"
        );
    }
    ...
    fn coverage_flags_a_structure_only_dod() {
        ...
        assert!(
            report.contains("coverage:") && report.contains("existence"),
            "{report}"
        );
        assert!(
        ...
        assert!(
            report2.contains("verified"),
            "the verified stratum is shown: {report2}"
        );
        assert!(
    ...
    fn wishlist_delta_flags_project_regression() {
        ...
        assert!(
            delta.newly_realized[0].contains("bb"),
            "{:?}",
            delta.newly_realized
        );
        let out = wishlist_delta_report(&delta, "delta since baseline", false);
        assert!(
            out.contains("regressed 1") && out.contains("newly realized 1"),
            "{out}"
        );
        assert!(
            out.contains("baseline 1"),
            "shows the prior aggregate: {out}"
        );
    }
    ...
    fn wishlist_delta_flags_counterfeit_progress() {
        ...
        assert!(
            !d1.has_regression(),
            "a hologram is not a status regression"
        );
        let out = wishlist_delta_report(&d1, "delta since baseline", false);
        assert!(
            out.contains("suspect 1") && out.contains("counterfeit"),
            "{out}"
        );
        ...
        assert!(
            d2.new_suspects.is_empty(),
            "an already-suspect wish is not newly suspect"
        );
    }
    ...
    fn apply_synthesis_routes_unscaffoldable_facet_to_fallback() {
        ...
        assert_eq!(
            apply_synthesis(&root, &dep, None, None, None, false).unwrap(),
            0
        );
    ...
    fn guided_redirects_a_naming_drift_facet_away_from_a_duplicate() {
        ...
        assert!(
            n > 0,
            "guided routes the near-miss to the provider to rename"
        );
    ...
    fn guided_architecture_note_warns_on_a_self_contradictory_dod() {
        ...
        let sound = vec![
            ...
            line(
                "e",
                WishFacet::new(WishFacetKind::Dependency, "api->logging"),
            ),
        ];
        assert!(
        ...
        let note = guided_architecture_note(&dangling, false).expect("a dangling wire is a defect");
        assert!(
