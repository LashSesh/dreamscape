...
/// Stub when the `plan` feature is off: the `pse-traverse` collapse-plan bridge is
/// an adapter dependency that is intentionally not extracted into standalone
/// lawish. `--plan` reports this rather than silently doing nothing.
#[cfg(not(feature = "plan"))]
pub fn collapse_plan_view(_label: &str, _facets: &[WishFacet]) -> Result<CollapsePlanView, String> {
    Err(
        "--plan requires building lawish with --features plan (which vendors \
         pse-traverse from the source repo); the default standalone build is PSE-free"
            .to_string(),
    )
}
...
mod tests {
    ...
    fn a_disconnected_component_is_excised() {
        ...
        assert!(
            view.excisions
                .iter()
                .any(|e| e.reason.contains("no_admissible_path")),
